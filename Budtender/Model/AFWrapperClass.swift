...
//            }
    ...
    func requestGETURL(_ strURL: String, params : [String : AnyObject]?,headers : HTTPHeaders?,  success:@escaping (AnyObject) -> Void, failure:@escaping (NSError) -> Void) {
        print(AppDefaults.token,"token ====>")
        print(strURL,"URL ====>")
            ...
            for (key, value) in params {
                ...
                if let temp = value as? [Data] {
                    for i in 0..<temp.count{
                        multiPart.append(temp[i] as! Data, withName: "post_images[\(i)]", fileName: "\(UUID().uuidString).png", mimeType: "image/png")
                    }

                }
                if let image = value as? Data{
                    multiPart.append(image, withName: key, fileName: "\(UUID().uuidString).png", mimeType: "image/png")
                }
            }
            print(multiPart)
    ...
    func sendMessage(strURL: String, params: [String: Any], completion:@escaping([String:Any]?,String?) -> Void) {
        print(params,"Param ====>")
        print(strURL,"URL ====>")
        print(AppDefaults.token,"token ====>")
        ...
        AF.upload(multipartFormData: { multiPart in
            for (key, value) in params {
                ...
                if let temp = value as? Data {
                    if  key == "document" {
                      multiPart.append(temp, withName: "document", fileName: "\(UUID().uuidString).mp4", mimeType: "video/mp4")
                    }else{
                        multiPart.append(temp, withName: key, fileName: "\(UUID().uuidString).png", mimeType: "image/png")
                    }
                }                
                if let temp = value as? [Data] {
                    if key == "chat_images" {
                        for i in 0..<temp.count {
                            multiPart.append(temp[i] as! Data, withName: "\(key)[\(i)]", fileName: "\(UUID().uuidString).png", mimeType: "image/png")
                        }
                        if let image = value as? Data {
                            multiPart.append(image, withName: key, fileName: "\(UUID().uuidString).png", mimeType: "image/png")
                        }
