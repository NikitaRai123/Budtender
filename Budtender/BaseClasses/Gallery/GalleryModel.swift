...
enum ImagePickerControllerType: String {
    ...
    var predicates: [NSPredicate] {
        switch self {
        case .both: return [NSPredicate(format: "mediaType = %d", PHAssetMediaType.video.rawValue),
...
class GalleryModel: NSObject {
    ...
    static func getFirstImage(complition:((UIImage?)->())?) {
        ...
        imageManager.requestImage(for: asset,targetSize: CGSize(width: deviceSize.width * sizeFactor, height: deviceSize.height * sizeFactor), contentMode: .aspectFit, options: nil, resultHandler: { (uiimage, info) in
            complition?(uiimage)
