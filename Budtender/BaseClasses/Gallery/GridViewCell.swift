...
//    }
    ...
    lazy var selectionImageView: UIView = {
        let img = UIImageView()
        img.contentMode = .scaleAspectFill
        img.image = #imageLiteral(resourceName: "SelectPhotoFeed")
        img.translatesAutoresizingMaskIntoConstraints = false
        return img
    }()
...
//        }
        ...
        imageManager.requestImage(for: asset,targetSize: CGSize(width: deviceSize.width * sizeFactor, height: deviceSize.height * sizeFactor), contentMode: .aspectFit, options: nil, resultHandler: { (image, info) in
            if self.representedAssetIdentifier == asset.localIdentifier {
