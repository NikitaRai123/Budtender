...
class GetImageFromPicker: NSObject {
    ...
    public func setImagePicker(imagePickerType: PickerType = .both,mediaType: MediaType, tag: Int = -1, controller: UIViewController?) {
        ...
        self.checkCameraAutorizationStatus(completion: {(isAuthorized) in
            DispatchQueue.main.async {
                ...
                if self.mediaType == .image {
                    self.openCameraForMediaType(.image, isEdit: true)
                } else if self.mediaType == .video {
                    self.openCameraForMediaType(.video, isEdit: false)
                }
    ...
    private func galleryAction() {
        self.checkGalleryAuthorizationStatus(completion: { (isAuthorized) in
            DispatchQueue.main.async {
                ...
                if self.mediaType == .image {
                    self.openGalleryForMediaType(.image, isEdit: true)
                } else if self.mediaType == .video {
                    self.openGalleryForMediaType(.video, isEdit: false)
                }
//                self.openGallery()
            }
        })
    ...
    private func openCameraForMediaType(_ mediaType: MediaType, isEdit: Bool) {
        self.imagePicker?.delegate = self
        self.imagePicker?.allowsEditing = isEdit
        ...
        if UIImagePickerController.isSourceTypeAvailable(.camera) {
            DispatchQueue.main.async {
                ...
                if mediaType == .image {
                    ...
                    self.imagePicker?.mediaTypes = MediaType.video.type
                    self.imagePicker?.videoMaximumDuration = 60 // Set maximum video duration
                }
                self.visibleController?.present(self.imagePicker!, animated: true, completion: nil)
            }
        } else {
    ...
    private func openGalleryForMediaType(_ mediaType: MediaType, isEdit: Bool) {
        self.imagePicker?.delegate = self
        self.imagePicker?.allowsEditing = isEdit
